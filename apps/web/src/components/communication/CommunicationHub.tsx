...
export function CommunicationHub({ officeHours = "Mon-Fri, 8 AM - 3 PM" }: CommunicationHubProps) {
    ...
    const [activeView, setActiveView] = useState<'feed' | 'thread' | 'ticket' | 'announcement'>('feed');
        ...
        <div className="relative w-full h-[100dvh] overflow-hidden bg-background md:max-w-4xl md:mx-auto md:border-x md:border-border/50 md:shadow-sm text-foreground">
            <AnimatePresence initial={false} mode="popLayout">
                {activeView === 'feed' ? (
                    ...
                    <DetailViewWrapper key="detail" onBack={handleBack}>
...
function FeedView({ onItemClick, officeHours, selectedChildId, setSelectedChildId, isTranslated, setIsTranslated }: FeedViewProps) {
        ...
        <ScreenStackBase>
            ...
            <div className="sticky top-[calc(3.5rem)] z-40 bg-background py-2 px-4 shadow-sm space-y-3 border-b border-border/30">
                ...
                <div className="flex gap-2">
                    ...
                    {/* Only show 'New' button to staff usually, keeping for visual consistency */}
                </div>
...
function DetailViewWrapper({ children, onBack }: { children: React.ReactNode, onBack: () => void }) {
    return (
        <ScreenStackDetail onBack={onBack}>
            {children}
        </ScreenStackDetail>
    );
}
...
function ScreenStackDetail({ children, onBack }: { children: React.ReactNode, onBack: () => void }) {
    ...
    return (
        ...
        >
            <div className="flex-1 flex flex-col h-full overflow-hidden">
                {/* Header */}
                <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-md border-b border-border/50 h-14 flex items-center px-4 shrink-0">
                    <button
                        onClick={onBack}
                        className="w-10 h-10 flex items-center justify-center -ml-2 rounded-full hover:bg-secondary/80 transition-colors"
                    >
                    ...
                    <div className="flex-1 text-center pr-8 font-bold text-lg truncate">
                        Details
                    </div>
                </header>
