...
export default function GradesClassesPage({ params }: Props) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const payload = {
                ...formData,
                capacity_max: Number(formData.capacity_max) || 30,
            };
        ...
        <div className="p-4 md:p-6 lg:p-8 max-w-7xl mx-auto space-y-5">
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-[hsl(var(--admin-text-sub))]">Manage classes, their grades, branches, and capacities.</p>
                </div>
                <button
                    type="button"
            ...
            <div className="ios-card p-4">
                <div className="flex flex-wrap items-end gap-4">
                    <div className="flex flex-col gap-1.5 min-w-[180px]">
                        <label className="text-xs font-medium text-[hsl(var(--admin-text-sub))]">Filter by Branch</label>
                        <select
                            value={filterBranch}
                            onChange={e => setFilterBranch(e.target.value)}
                            className="px-3 py-2 text-sm rounded-xl border border-[hsl(var(--admin-border))] bg-[hsl(var(--admin-surface))] text-[hsl(var(--admin-text-main))] outline-none"
                        >
                    ...
                    <div className="flex flex-col gap-1.5 min-w-[180px]">
                        <label className="text-xs font-medium text-[hsl(var(--admin-text-sub))]">Filter by Grade</label>
                        <LookupSelect
                            label=""
                            value={filterGrade}
                            onChange={v => setFilterGrade(v as string)}
                            dictName="grades"
                    ...
                    {(filterBranch || filterGrade) && (
                        <button
                            type="button"
                            onClick={() => { setFilterBranch(''); setFilterGrade(''); }}
                            className="px-3 py-2 text-xs font-semibold text-[hsl(var(--admin-text-sub))] hover:text-[hsl(var(--admin-text-main))] transition-colors"
                        >
                            Clear Filters
                        </button>
                    )}
                ...
                <div className="ios-card p-5 space-y-4 border-2 border-blue-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <LookupSelect
                            ...
                            onChange={v => updateField('grade_code', v)}
                            dictName="grades"
                            required
                        />
                    ...
                    <div className="flex items-center gap-3 pt-2">
                        <button
                            type="button"
                            onClick={handleSave}
                            disabled={saving || !formData.class_name || !formData.grade_code || !formData.branch_id}
            ...
            {/* List */}
            {loading ? (
                <div className="ios-card p-8 text-center text-[hsl(var(--admin-text-muted))] text-sm">Loading classes...</div>
            ) : items.length === 0 ? (
                <div className="ios-card p-8 text-center text-[hsl(var(--admin-text-muted))] text-sm">
                    No classes found. {filterBranch || filterGrade ? 'Try adjusting your filters or c' : 'C'}lick "Add Class" to create one.
                </div>
            ) : (
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-[hsl(var(--admin-border))]">
                                <th className="text-left text-xs font-semibold text-[hsl(var(--admin-text-muted))] uppercase tracking-wider py-3 px-4">Class Name</th>
                        ...
                        <tbody>
                            {items.map(item => (
                                <tr key={item.id} className="border-b border-[hsl(var(--admin-border))] hover:bg-[hsl(var(--admin-surface))] transition-colors">
                                    <td className="py-3 px-4 text-sm font-medium text-[hsl(var(--admin-text-main))]">{item.class_name}</td>
                                    <td className="py-3 px-4">
                                        <span className="px-2 py-1 bg-blue-50 text-blue-700 rounded-lg text-xs font-medium">{item.grade_code}</span>
                                    </td>
                                    <td className="py-3 px-4 text-sm text-[hsl(var(--admin-text-sub))]">{item.branch_name || item.branch_id}</td>
                                    <td className="py-3 px-4 text-center">
                                        <span className="text-sm text-[hsl(var(--admin-text-main))]">
